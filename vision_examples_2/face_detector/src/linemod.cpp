...
static void writeLinemod(const cv::Ptr<cv::linemod::Detector>& detector, const std::string& filename)
{
  ...
  for (int i = 0; i < (int)ids.size(); ++i)
  {
    ...
    //TODO TODO ADD STUFF HERE ADAM for depth
    fs << "}"; // current class
  }
  fs << "]"; // classes
}
...
void load()
{
  ...
  // if (argc == 1)
  // {
    // ripped off from getDefaultLINEMOD()
    ...
    // detector = cv::linemode::getDefaultLINEMOD();

  // }
  // else
  // {
    detector = readLinemod("linemod_templates.yml");

    std::vector<cv::String> ids = detector->classIds();
    num_classes = detector->numClasses();
    printf("Loaded linemod_templates.yml with %d classes and %d templates\n",
  ...
  // }
  std::cout << "linemod done loading" << std::endl;
}
...
void doit(const cv::Mat& color_in, cv::Mat& depth_in)
{
  ...
  // {
    ...
    // cv::Mat color;
    // if (color_in.rows > 960)
    //   cv::pyrDown(color_in.rowRange(0, 960), color);
    // else
